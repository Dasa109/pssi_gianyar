    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="lg:sticky lg:top-24 space-y-6"> 
                <div class="bg-zinc-800 border-l-4 border-red-600 p-6 shadow-xl">
                    ...
                    <div class="space-y-5">
                        <div class="flex justify-between border-b border-white/5 pb-2">
                            ...
                            <span class="text-white font-bold bg-zinc-900 px-2 py-1 rounded text-sm">{{ $club->short_name ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between border-b border-white/5 pb-2">
                            <span class="text-zinc-500 text-xs font-bold uppercase tracking-widest">Kontak</span>
                            <span class="text-white font-bold text-sm">{{ $club->phone ?? '-' }}</span>
                        </div>
                        <div class="pb-2">
                ...
                <a href="{{ route('clubs.index') }}" class="group block w-full text-center py-4 border border-zinc-700 text-zinc-400 hover:text-white hover:border-red-600 hover:bg-red-600/10 transition uppercase font-bold text-xs tracking-[0.2em]">
                        ...
                        @foreach($club->players as $player)
                            <div class="relative group overflow-hidden rounded-xl bg-zinc-800 border border-zinc-700/50 hover:border-red-600 hover:shadow-[0_0_20px_rgba(220,38,38,0.2)] transition-all duration-300">
                                <div class="h-64 overflow-hidden bg-gradient-to-b from-zinc-700 to-zinc-900 relative">
                                    @if($player->photo)
                                        <img src="{{ asset('storage/' . $player->photo) }}" class="w-full h-full object-cover object-top group-hover:scale-110 transition duration-500">
                                    @else
                                ...
                                <div class="absolute bottom-0 left-0 w-full p-4 transform translate-y-2 group-hover:translate-y-0 transition duration-300">
                                    <div class="flex items-end justify-between">
                                        <div>
                                            ...
                                            @endif
                                            <h4 class="text-white font-bold text-lg leading-tight uppercase font-heading">{{ $player->name }}</h4>
                                        </div>
